 ...
 */
public class FMCSyncUnAssignJob implements Job {
    static Logger logger = Logger.getLogger(FMCSyncUnAssignJob.class);
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            try {
                ...
                unassign(nRetries, parentId, childSerial, userId, groupId, timeStamp, tCode, operationId, sapId);
            } catch (Exception ex) {
                nRetries++;
    ...
    public void unassign(int nRetries, Long parentId, String childSerial, Long userId, Long groupId, Long timestamp, String tCode, long operationId, long sapId) throws Exception {
        ...
        fmcHandleSapSendMessage(EVENT_TYPE_DISASSOCIATE, message, messageDetail, parent, user, sapSyncStoreDate, handHeldEventDate, nRetries, tCode, operationId, sapId);
    }
    ...
    public static void reschedule(int retry, Long parentId, String childSerial, Long userId, Long groupId, Long timestamp, String tCode, long operationId, Long sapId) {
        ...
        if (parentId != null && childSerial != null && userId != null && groupId != null) {
            ...
            JobKey jobKey1 = new JobKey("fmcSyncSapUnAssignJob_" + jobKey, "fmcSyncSAPJob");
            if (retry < getMaxNumberOfRetries(user)) {
                JobDetail job = newJob(FMCSyncUnAssignJob.class)
                        .withIdentity("fmcSyncSapUnAssignJob_" + jobKey, "fmcSyncSAPJob")
                        .build();

                Trigger trigger = newTrigger()
                        .withIdentity("fmcSyncSapUnAssignTrigger_" + jobKey, "fmcSyncSAPJob")
                ...
                logger.error("Task fmcSyncSapUnAssignJob_"+jobKey+ " cannot be reschedule the maximum # of retries has been used ");
