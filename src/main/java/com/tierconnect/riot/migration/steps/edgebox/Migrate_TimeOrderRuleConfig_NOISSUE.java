 ...
 * on 11/17/15.
 */
public class Migrate_TimeOrderRuleConfig_NOISSUE implements MigrationStep {
    private static Logger logger = Logger.getLogger(Migrate_TimeOrderRuleConfig_NOISSUE.class);
    ...
    public void migrateHibernate() throws Exception {
        migrateTimeOrderRule();
    }

    private void migrateTimeOrderRule() {

        List<Edgebox> edgeboxes = EdgeboxService.getInstance().getByType("core");
        for (Edgebox edgebox : edgeboxes) {
            if (!edgebox.getConfiguration().contains("timeOrderRule")) {
                ...
                try {
                    ...
                    ((ObjectNode) rootNode).set("timeOrderRule", timeOrderRule);

                    edgebox.setConfiguration(rootNode.toString());
                    EdgeboxService.getEdgeboxDAO().update(edgebox);
                    logger.info("Updated CoreBridge configuration, timeOrderRule config added");
