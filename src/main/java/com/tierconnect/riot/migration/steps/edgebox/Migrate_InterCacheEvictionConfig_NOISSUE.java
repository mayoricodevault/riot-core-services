 ...
 * on 11/17/15.
 */
public class Migrate_InterCacheEvictionConfig_NOISSUE implements MigrationStep {
    private static Logger logger = Logger.getLogger(Migrate_InterCacheEvictionConfig_NOISSUE.class);
    ...
    public void migrateHibernate() throws Exception {
        migrateIntrCacheEviction();
    }

    private void migrateIntrCacheEviction() {

        List<Edgebox> edgeboxes = EdgeboxService.getInstance().getByType("core");
        for (Edgebox edgebox : edgeboxes) {
            if (!edgebox.getConfiguration().contains("interCacheEviction")) {
                ...
                try {
                    ...
                    ((ObjectNode) rootNode).set("interCacheEviction", interCacheEviction);

                    edgebox.setConfiguration(rootNode.toString());
                    EdgeboxService.getEdgeboxDAO().update(edgebox);
                    logger.info("Updated CoreBridge configuration, interCacheEviction config added");
