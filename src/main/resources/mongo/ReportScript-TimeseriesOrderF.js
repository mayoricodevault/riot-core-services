...
function (options) {
   ...
   function timestampToHHMM(timestamp) {
    ...
    table.title = "ViZix Timeseries Out of Order Upsert Rate, by Filter, External Timestamp";
                                   ...
                                   },
                                   {
                                       "$project": {
                                           ...
                                           "filter": {
                                                            ...
                                                            {
                                                                "$and": [
                                                                    {"$ne": ["$value.filter.value", null]},
                                                                    {"$ne": ["$value.filter.value", undefined]}
                                                                ]
                                                            },"$value.filter.value",
                                                            "filterNotExists"]
                                            }
    ...
    table.data = [];
    for (var rowKey = 0; rowKey < table.rowNames.length; rowKey++) {
        var rowResult = [];
        var total = 0.0;
        for (var colKey = 0; colKey < table.columnNames.length; colKey++) {
