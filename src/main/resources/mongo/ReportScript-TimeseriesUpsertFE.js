...
function (options) {
    ...
    table.title = "ViZix Timeseries Upsert Rate, External Timestamp, by Filter";
        ...
        [
            {
                "$match": {
                    ...
                    "value.filter.value" : { "$exists": true },
                    /*"value.tsCoreIn.value": {$gte: start, $lte: end}
                    "value.tsCoreIn" : { $exists: true }*/
                }
            },
            {"$project" : {
                "_id" : "$_id",
                "serialNumber" : "$value.serialNumber",                
                "source" : "$value.filter.value",      
                "time" : "$time"
            }},
            {"$sort": {"time": 1}},
            {
